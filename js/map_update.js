    ...
    //insert the objects in as a card in the html
        htmlString +=   '<div class="meldung_container_style_bearbeiten" id="meldung'+x+'">'+
                            ...
                            '<div>'+
                                ...
                                '<p id="status"><b>Status:</b> <select id="div-status'+x+'" name="status">'+
                                ...
                                '<p class="kommentar-style" id="kommentar"><b>Kommentar:</b>  <input placeholder="Maximal 200 Zeichen" maxlength=200 class="kommentar-style-input" id="div-kommentar'+x+'"></p>'+ 
                                '</text>'+
                                '<button class="div-delete"> Änderung Speichern</button><button class="div-delete">Meldung Löschen</button> '+
                            '</div>'+
                        '</div>'
                        ;
}
...
window.onload=function(){
  ...
  for(let i = 0; i < json.length;i++){
    ...
    //@Peter catch the changed input from here when the user makes updates
    //document.querySelector("#div-kategorie"+[i]).value = nobj.kategorie;
    //document.querySelector("#div-beschreibung"+[i]).value = nobj.beschreibung;
    document.querySelector("#div-status"+[i]).value = nobj.status;
